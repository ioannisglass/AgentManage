...
const Toastr = ({ type , text, start, handleClose }) => {
    ...
    useEffect(() => {
        if(flg) {
            if (start === true) {
                animationCtrl.set({ opacity: 0 })
                animationCtrl.start({ opacity: 1})
            }
            else {
                animationCtrl.set({ opacity:1})
                animationCtrl.start({ opacity:0})
            }
        }
        setFlg(true);
    }, [start])
    ...
    return (
        ...
        >
            <div 
                className="fixed flex top-5 right-5 items-center w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" 
                role="alert"
            >
